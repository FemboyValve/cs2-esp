	...
	void loop() {
		...
		if (g_game.isC4Planted) {
			...
			if (c4ScreenPos.z >= 0.01f) {
				float c4Distance = g_game.localOrigin.distance_to(c4Origin);
		...
		for (auto player = g_game.m_players.begin(); player < g_game.m_players.end(); player++) {
			...
			float distance = g_game.localOrigin.distance_to(player->origin);
